...
- 结果：用报告里**每支股票最新一根日线**判断，命中的按成交额从高到低列出来，每一行附上条件里的数值 (例如「RSI(14) 28.3」「相对量(20) 2.4」)，点一行打开完整图表 + 财报
- **回测这组条件**：用每支股票最近约 6 个月的日线，看条件"由不成立变成立"之后 5 / 10 / 20 天平均涨跌、胜率，跟同期任意一天买进比 (在浏览器里算，没扣交易成本)
- **内置选股策略**：后台默认策略（跟后台信号同一套条件，结果一致）、RSI 超卖回升、均线金叉、放量突破、MACD 金叉、Supertrend 转多
- 模板存在自己的浏览器里；「我的模板」里可以改名、**导出备份 / 导入备份**（换手机或电脑时用）

**策略回测**：「后台信号」下面有一块回测，用近 6 个月日线把后台策略每一天都判断一次 (信号当天收盘后，隔天开盘价计入；之后收盘跌到 SAR 以下就结算，最多 20 个交易日；每笔扣来回交易成本约 0.5%，美股 0.1%)：
- 胜率、每笔平均 (中位数)、盈亏比、平均持有几天
- 信号之后固定 5 / 10 / 20 天的涨跌，跟**同期任意一天买进**的平均比 (看信号有没有比随便买好)
- 风险与报酬：初始风险 (计入价到 SAR)、期间最大涨幅 / 跌幅、平均 R 倍数、获利因子、最好 / 最差一笔、最多连亏、结算原因
- 每笔收益分布，以及**最近 20 个交易日的信号现在怎样** (持有中还是已结算、收益多少)，点一行打开那支股票
- 只统计今天进报告的股票 (有幸存者偏差)，历史统计不代表未来；ⓘ 有完整说明

**后台信号**：后台用固定策略、同时满足全部四个条件的股票，每支一张卡片，做成**左右翻阅的卡片轮播**：
- **上榜理由带数字**：例如「EMA20多头 +2.8% · SAR多头 +5.2% · T3形态突破 · 量 2.1×」(现价比 EMA20 / SAR 高多少、今天量是平时几倍)；交易时间内生成的报告标「盘中」(日线还没收完，收盘前信号可能消失)
...
- **图表下方**：十字光标所在那根 K 线的时间、开高低收、涨跌、成交量，以及成交额、相对量、RSI(14)、EMA20、50 日均线、SAR (多空 + 价位)、ATR(14) 波动、**风险 (到 SAR)** = 现价跌到 SAR 要跌多少、**风险报酬比** = 回测里同类信号期间最大涨幅的中位数 ÷ 风险
...
|---|---|
| # | 序号，排序或筛选后自动重新从 1 编号 |
| 股票 | 简称 + 股票代码；新股标「上市 N 天」(天数不够算的指标显示「—」)；马股低价股标「跳一格 N%」(0.035 的股票跳一格就是 14%) |
...
默认**按成交额**从高到低排 (马股低价股一天成交几亿股但金额很小，按股数排会误导)，点表头可以换排序。上方的**快速筛选**可以叠加：★ 自选、SAR 多头、放量 ≥ 2×、RSI < 30、RSI > 70、排除 RM0.10 以下 (美股 $5 以下)。电脑上表格撑满整个页面宽度；手机上每支股票排成两行刚好塞进屏幕（第一行：名称、走势、价格；第二行：成交额、相对量、RSI、SAR、EMA20、涨跌%），数字用短格式 (81.1M) 不会挤在一起，排序改用下拉框。

**点任何一行**（键盘选中后按 Enter）打开这支股票的完整图表和财报：

### 完整图表 + 财报 + 公告 + 新闻
- **最上面**：☆ 加自选、计算器、分享 (手机上用系统分享，电脑上复制链接)，右边 ‹ › 按打开时那个列表的顺序 (表格现在的排序和筛选、命中列表、榜单…) 换上一支 / 下一支
- **关键数字**：市值、市盈率、股息率、52 周区间 (小条上的点 = 现价位置)、下次财报日期 (两周内会标出来)、20 日平均成交额、ATR 波动、一手 (100 股) 多少钱
...
### 名词解释 (☰ → 工具，或页面上的 ⓘ)
当前价格、盘中 / 已收盘、成交额、相对量、RSI、EMA20、SAR、T3 形态、ATR、风险报酬比、回测的胜率 / 盈亏比 / 获利因子 / R 倍数、跳一格、52 周区间、市盈率 / 股息率的白话说明。
...
- 🎯 **固定量化策略**：`SAR 多头` + `EMA20 < 现价` + `成交量达标` + `T3 形态突破` 四个条件同时满足才算命中。成交量门槛按价格分级，越便宜的股票要求越高：
  ...
  | 0.20 – 0.50 | 100 万 |
  | 0.50 以上 | 50 万 |
- 🧪 **策略回测 + 风险报酬**：用近 6 个月日线逐日回测后台策略 (胜率、盈亏比、获利因子、R 倍数、收益分布、最近 20 个交易日的信号表现)，跟同期任意一天买进比；每张信号卡片标出到 SAR 的风险和风险报酬比；自己的选股条件也能一键回测
...
- 📊 **选股器式表格**：每支股票都有日内迷你走势图、成交额、相对成交量、多空标签，默认按成交额排，快速筛选可以叠加；点任何一行看完整 K 线图
