...
- ⚙️ **网页端自定义**：图表颜色可以直接在报告页面上调整；内置 18 个常用指标（SAR、布林带、RSI、MACD、KD、CCI、ATR、OBV、VWAP 等），按趋势/动量/波动性/成交量分类，点一下就叠加到图上；也能自己输入公式（如 `sma(close,10)`）生成新指标。全部即时生效，保存在你自己的浏览器里
